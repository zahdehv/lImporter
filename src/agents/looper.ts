...
export async function run_node(plugin: lImporterPlugin, chat: Chat, initMessage: PartListUnion, functions?: FunctionArg[], max_turns = 7, max_retries = 3) {
        ...
        for await (const chunk of response) {
            if (functions && chunk.functionCalls?.length && chunk.functionCalls?.length>0) {
                for (let index = 0; index < chunk.functionCalls.length; index++) {
                    ...
                    if (fx){
                        const res = await fx.run(plugin, fc.args)
                        ...
                        messages.push(prt);
                        // plugin.tracker.createMessage("AI").MD(FORMAT_CALLOUT("info", '-', "called "+fc.name+" with "+fc.args));
                    }
                }
            }
            if (chunk.text) {
